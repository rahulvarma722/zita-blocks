...
export default class LayoutsProvider extends Component {
    ...
    all: [
      {
        category: ["business"],
        content:
          "<!-- wp:zita-blocks/zita-blocks-layouts /--><!-- wp:paragraph --><p>hello chck para 123</p><!-- /wp:paragraph -->",
        image:
          '"https://demo.studiopress.com/page-builder/gb-layout-business-1.jpg"',
  ...
  //   })
  //     .then((favorite_keys) => {
  //       console.log("layouts provider favorite_keys", favorite_keys);
  //       return favorite_keys;
  //     })
  //     .catch((error) => console.error("getFavoriteKeys->", error));
  // }
  ...
  getFavorites() {
    return apiFetch({
      ...
      .then((favorite_keys) => {
        const favorites = [];
        console.log("layouts provider getFavorites", favorite_keys);
  ...
  render() {
    console.log("layout provider called");
    console.log("his.state", this.state);
    console.log("this.props provider", this.props);

    if (this.state.all) {
      console.log("inside layout provider called");
      return (
