...
if (!function_exists('zita_blocks_post_grid_block')) {
    function zita_blocks_post_grid_block($attr)
    {
        ...
        if (isset($attr['numberOfPosts']) && intval($attr['numberOfPosts'])) {
            ...
            if ($query->have_posts()) {
                ...
                $metaBlockBgColor = isset($attr['meta_style'][0]['blockBgColor']) ? $attr['meta_style'][0]['blockBgColor'] : '';
                $postHtml .= '<div class="zita-block-post zita-image-section" id="zita-block-post" style="background-color:' . $metaBlockBgColor  . ';">';
                // post title
                // loader
